...
export class Reel extends GameObject {
  ...
  private moveReel(ticker: Ticker): void {
    ...
    // also, i wanted to copy the original spin design where the symbol position remain static as the reel spins
    ...
    // trying lower values in Constants.maxReelSpeed results in more standard spinning
  ...
  private updateSymbolTexture(symbolSprite: Sprite): void {
    switch (this.state) {
      ...
      case ReelState.STOPPING:
        ...
        // in this case we want to cascade the textures AFTER updating first symbolSprite
